...
public class LR extends JFrame implements ActionListener{
	...
	public void init()
	{
		...
		jl2 = new JLabel("请输入以'#'结尾的待分析串:");
		jl2.setBounds(320, 30, 320, 30);
		...
		jb3 = new JButton("输入");
		jb4 = new JButton("清空");
		jb3.setBounds(842, 30, 60, 25);
		jp1.add(jb3);
		jb4.setBounds(900, 30, 60, 25);
		...
		jscroll.setBounds(340, 80, 620, 160);
		jscroll.setOpaque(false);
		jscroll.getViewport().setOpaque(false);
		jp1.add(jscroll);
		
		
		jb5 = new JButton("生成LR分析表");
		jb5.setBounds(340, 250, 200, 40);
		jb5.setForeground(Color.white);
		jb5.setFont(new Font("微软雅黑", 0, 25));
		jb5.setOpaque(false);
		...
		jt1 = new JTable();
		jt2 = new JTable();
		jt1.setBounds(550, 250, 200, 250);
		jt2.setBounds(750, 250, 200, 250);
		jt1.setOpaque(false);
		jt2.setOpaque(false);
		jp1.add(jt1);
		jp1.add(jt2);
				...
				while(!(k=="1"&&t=='#'))
				{
					...
					else
					{
						...
						if(result1.charAt(0)=='s')
						{
							...
							state_num++;
							char_stack[char_num]=zifu[fzifu-1];
							char_num++;
							fzifu--;
						}
						else if(result1.charAt(0)=='r')
						{
							...
							strbuf = null;
							int len = rule[index].length()-2;
							state_num = state_num - len;
							char_num = char_num - len;
							String gox ;
							int a;
							int b;
							//取出出栈后的首个状态值  作为行
						...
						//		JOptionPane.showMessageDialog(null,"分析失败！！  该串不是该文法的句子!");
							...
							else
							{
								...
								char_num++;
								char_stack[char_num-1] = rule[index].charAt(0);
								state_num++;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==jb5)
		{
			if(flag!=0)
			{
				...
				jtable1.setBounds(550, 250, 200, 250);
				...
				jtable2.setBounds(750, 250, 200, 250);
				jp1.add(jtable1);
				jp1.add(jtable2);
			}
			else
...
class NewPanellr extends JPanel
{
	/**
	 * @author CRR
	 */
